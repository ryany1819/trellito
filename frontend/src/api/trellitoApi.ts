...
export const trellitoApi = createApi({
	...
	endpoints: (builder) => ({
		...
		getMe: builder.query<User, void>({
			...
			transformResponse: (response: { user: UserDto }): User =>
				mapUserFromDto(response.user),
		}),
		...
		getUsersByIds: builder.query<User[], string[]>({
			...
			transformResponse: (response: { users: UserDto[] }): User[] =>
				response.users?.map(mapUserFromDto) || [],
		}),
	}),
});
